...
window.configure(background='#AA5203')  # Updated to brijal color (indigo shade)
...
def manually_fill():
    ...
    sb.geometry('600x350')
    sb.configure(background='#AA5203')
    ...
    def err_screen_for_subject():
        ...
        ec.geometry('300x100')
        ec.title('Warning!!')
        ec.configure(background='#4B0082')
        Label(ec, text='Please enter your course name!!!', fg='white',
              bg='#800080', font=('times', 20, ' bold ')).pack()
        Button(ec, text='OK', command=ec_delete, fg="white", bg="#32CD32", 
    ...
    def fill_attendance():
        ...
        DB_table_name = f"{subb.replace(' ', '_')}_{Date}_Time_{Hour}_{Minute}_{Second}"

        # Connect to the database
        try:
            global cursor, connection
            connection = sqlite3.connect('manually_fill_attendance.db')
        ...
        MFW.configure(background='lightgrey')
        ...
        def err_screen1():
            ...
            errsc2.configure(background='#4B0082')
            Label(errsc2, text='Please enter Student & Enrollment!!!', fg='white', 
                  bg='#800080', font=('times', 16, ' bold ')).pack()
            Button(errsc2, text='OK', command=del_errsc2, fg="white", bg="#32CD32", 
                   width=9, height=1, activebackground="Red",
                   font=('times', 15, ' bold ')).place(x=90, y=50)
        ...
        def enter_data_DB():
            ...
            time_now = datetime.datetime.fromtimestamp(ts).strftime('%H:%M:%S')
            Insert_data = f"INSERT INTO {DB_table_name} (ID, ENROLLMENT, NAME, DATE, TIME) VALUES (NULL, ?, ?, ?, ?)"
            VALUES = (ENROLLMENT, STUDENT, Date, time_now)
            try:
        ...
        def create_csv():
            ...
            csv_dir = 'Attendance_management_system/Attendance/'
            os.makedirs(csv_dir, exist_ok=True)
            csv_name = os.path.join(csv_dir, f"{DB_table_name}.csv")
            with open(csv_name, "w", newline="") as csv_file:
                csv_writer = csv.writer(csv_file)
                csv_writer.writerow([i[0] for i in cursor.description])  # Write headers
                csv_writer.writerows(cursor)
            Notifi.configure(text="CSV created Successfully", bg="#32CD32", fg="white")
            Notifi.place(x=180, y=380)

        Notifi = tk.Label(MFW, text="", bg="#32CD32", fg="white", width=33, height=2, font=('times', 19, 'bold'))

        clear_enroll = tk.Button(MFW, text="Clear", command=remove_enr, fg="white", bg="#4B0082", 
                                 width=10, height=1, activebackground="white", font=('times', 15, ' bold '))
        clear_enroll.place(x=690, y=100)

        clear_student = tk.Button(MFW, text="Clear", command=remove_student, fg="white", bg="#4B0082", 
                                  width=10, height=1, activebackground="white", font=('times', 15, ' bold '))
    ...
    SUB = tk.Label(sb, text="Enter Subject : ", width=15, height=2, fg="white", bg="#4B0082", 
                   font=('times', 15, ' bold '))
    ...
    fill_manual_attendance = tk.Button(sb, text="Fill Attendance", command=fill_attendance, fg="black", bg="SkyBlue1", 
                                       width=20, height=2, activebackground="white", font=('times', 15, ' bold '))
...
def take_img():
    ...
    if l1 == '':
        ...
        try:
            ...
            detector = cv2.CascadeClassifier(r'C:\Users\vishn\OneDrive\Documents\Attendance_Management_System\Attendance_Management_System\haarcascade_frontalface_default.xml')
...
def subjectchoose():
    def Fillattendances():
        ...
        if time.time() < future:
            if sub == '':
                ...
                try:
                    global cursor
                    connection = sqlite3.connect('Face_reco_fill.db')
                    cursor = connection.cursor()
                except Exception as e:
                    print(e)
                ...
                insert_data = "INSERT INTO " + DB_Table_name + \
                    " (ID,ENROLLMENT,NAME,DATE,TIME) VALUES (NULL, ?, ?, ?,?)"
                ...
                cs = 'C:/Users/Admin/OneDrive/Desktop/Attendance_Management_System/Attendance/' + fileName
    ...
    windo.configure(background='#AA5203')
    ...
    def Attf():
        import subprocess
        subprocess.Popen(
            r'explorer /select,"Attendace_management_system\Attendance\-------Check atttendance-------"')
    ...
    def log_in():
        ...
        if username == 'raditya':
            if password == 'raditya123':
                ...
                cs = "C:\\Users\\Admin\\OneDrive\\Desktop\\Attendance_Management_System\\StudentDetails\\StudentDetails.csv"
                with open(cs, newline="") as file:
                    reader = csv.reader(file)
                    r = 0
...
message = tk.Label(window, text="Attendance Management System using Face Recognition", bg="#FFD900", fg="black", width=50,
